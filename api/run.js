...
async function addLecture(title,id){
...
async function addSection(title,id){
...
try{
 ...
 for(const l of selectedLectures){
  await addLecture(getTitle(l),l.id);
 }

 for(const s of selectedSection){
  await addSection(getTitle(s),s.id);
 }
